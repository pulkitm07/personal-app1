...
Return ONLY a strictly valid JSON array of 3 objects. Do not include markdown blocks like \`\`\`json.
Keys needed: 
...
Return ONLY a strictly valid JSON array of 2 objects. Do not include markdown blocks like \`\`\`json.
Keys needed: 
...
Return ONLY a strictly valid JSON object representing ONE book. Do not include markdown blocks like \`\`\`json.
Keys needed: 
"title" (string), "author" (string), "category" (string), "year" (number), "summary" (string), "highlights" (array of exactly 5 string highlights), "action" (string "One Thing to Do Today").`;
    ...
    const data = await callOpenAI(prompt, 'daily_book_cache', dateStr, async () => {
      // Hardcoded fallback pulled exactly from previous code to ensure nothing is lost during an API crash
      return {
...
Return ONLY a strictly valid JSON object representing ONE company case study. Do not include markdown blocks like \`\`\`json.
Keys needed: 
"company" (string), "type" (string: exactly "rise", "fall", or "comeback"), "headline" (string), "founded" (number), "country" (string), "industry" (string), "story" (string), "diagnosis" (string), "lessons" (array of exactly 3 strings).`;
    ...
    const data = await callOpenAI(prompt, 'daily_case_cache', dateStr, async () => {
      // Hardcoded fallback pulled exactly from previous code
      return {
        company: 'WeWork',
        type: 'fall',
